...
proc initCamera*(lookFrom, lookAt, vup: Vec3, vfov, aspectRatio, aperture, focusDist: float64): Camera =
    ...
    result.lowerLeftCorner = result.origin - (result.horizontal / 2) - (result.vertical / 2) - (w * focusDist)
    result.u = u
    result.v = v
    result.w = w
...
proc ray*(c: Camera, s, t: float64): Ray =
    ...
    return initRay(c.origin + offset, c.lower_left_corner + (c.horizontal * s) + (c.vertical * t) - c.origin - offset)
