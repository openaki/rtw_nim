...
proc main() =
    ...
    let materialLeft = MaterialType(kind: LambertianKind, l: Lambertian(albedo: initVec(0.0,0.0,1)))
    let materialRight = MaterialType(kind: LambertianKind, l: Lambertian(albedo: initVec(1,0,0.0)))

    let s4 = Sphere(center: initVec(-R,0,-1), radius: R, material: materialLeft)
    let s5 = Sphere(center: initVec(R,0,-1), radius: R, material: materialRight)
