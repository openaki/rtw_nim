...
proc main() =
    ...
    let materialGround = MaterialType(kind: LambertianKind, l: Lambertian(
            albedo: initVec(0.8, 0.8, 0)))
    let materialCenter = MaterialType(kind: LambertianKind, l: Lambertian(
            albedo: initVec(0.1, 0.2, 0.5)))
    ...
    let materialLeft = MaterialType(kind: DielectricKind, d: Dielectric(
            indexOfRefraction: 1.5))
    let materialRight = MaterialType(kind: MetalKind, m: Metal(albedo: initVec(
            0.8, 0.6, 0.2), fuzz: 0))

    let s1 = Sphere(center: initVec(0, -100.5, -1), radius: 100,
            material: materialGround)
    let s2 = Sphere(center: initVec(0, 0, -1), radius: 0.5,
            material: materialCenter)
    let s3 = Sphere(center: initVec(-1, 0, -1), radius: 0.5,
            material: materialLeft)
    let s4 = Sphere(center: initVec(-1, 0, -1), radius: -0.4,
            material: materialLeft)
    let s5 = Sphere(center: initVec(1, 0, -1), radius: 0.5,
