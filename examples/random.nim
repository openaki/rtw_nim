...
proc randomScene(): HittableList =
    let materialGround = MaterialType(kind: LambertianKind, l: Lambertian(albedo: initVec(0.5,0.5,0.5)))
    ...
    for a in countup(-11, 11):
        for b in countup(-11, 11):
            ...
            let center = initVec(float64(a) + 0.9*rand(1.0), 0.2, float64(b) + 0.9*rand(1.0))
